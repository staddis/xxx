...
// newShareCodes = [ { "inviteCode":"Ih4-bOuwb_0l8G-6iw"}] //这里修改你的邀请码
newShareCodes = [{ "inviteCode":"JBk_Zeu0YP878m3SyHMR"}] //这里修改你的邀请码
if ( $.isNode() ) {
...
function getCookieInviteCode (cookieName,cookie) {
  return new Promise( ( resolve ) => {
    $.get( taskUrl2( "cash_getJDMobShareInfo", { "source": 2 }, cookie ), ( err, resp, data ) => {
      try {
        if ( err ) {
          console.log( `${ JSON.stringify( err ) }` )
          console.log( `${ $.name } API请求失败，请检查网路重试` )
          ...
          if ( safeGet( data ) ) {
            ...
            if ( data.code === 0 && data.data.bizCode === 0 ) {
              // console.log( `你的inviteCode: ${ data.data.result.inviteCode }` )
              // console.log( `你的shareDate: ${ data.data.result.shareDate }` )
              console.log( `【${ cookieName}】的inviteCode: ${ data.data.result.inviteCode }` )
              let helpInfo = {
                'inviteCode': data.data.result.inviteCode,
                'shareDate': data.data.result.shareDate
              }
...
function taskUrl2 ( functionId, body = {}, cookie ) {
  return {
    url: `${ JD_API_HOST }?functionId=${ functionId }&body=${ escape( JSON.stringify( body ) ) }&appid=CashRewardMiniH5Env&appid=9.1.0`,
    headers: {
      'Cookie': cookie,
