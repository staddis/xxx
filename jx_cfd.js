  ...
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        ...
        `ptag=7155.9.47&dwReqUserFlag=${dwUserFlag}&ddwMoney=${ddwMoney}`,
        `_cfd_t,bizCode,ddwMoney,dwEnv,dwReqUserFlag,ptag,source,strZone`
      ),
      async (err, resp, data) => {
        try {
          //$.log(data)
...
function doTreasureHunt(place) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`consume/TreasureHunt`, `ptag=7155.9.47&strIndex=${place}&dwIsShare=0`, `_cfd_t,bizCode,dwEnv,dwIsShare,ptag,source,strIndex,strZone`),
      async (err, resp, data) => {
        try {
          //$.log(data);
          if (!data || data.startsWith('<')) {
...
function doTask(taskinfo) {
  return new Promise(async (resolve) => {
    const { taskId, completedTimes, configTargetTimes, taskName, userStatusConfigExtra } = taskinfo;
    if (parseInt(completedTimes) >= parseInt(configTargetTimes)) {
      resolve(false);
      $.log(`\n${taskName}【做日常任务】： mission success`);
      return;
    }
    if (userStatusConfigExtra.length > 0) {
      var configExtra = userStatusConfigExtra.split(',');
      for (let i = 1; i <= configExtra; i++) {
...
function taskUrl(function_path, body, stk) {
  let url = `${JD_API_HOST}jxcfd/${function_path}?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now()}&${body}&_ste=1&_=${Date.now()}&sceneval=2&g_login_type=1&g_ty=ls`;
  if (stk) {
    url += `&_stk=${stk}`;
  }
...
function taskListUrl(function_path, body, stk) {
  let url = `${JD_API_HOST}newtasksys/newtasksys_front/${function_path}?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=&${body}&_ste=1&_=${Date.now()}&sceneval=2&g_login_type=1&g_ty=ls`;
  if (stk) {
    url += `&_stk=${stk}`;
  }
